...
def backup():
    ...
    if not os.path.exists(source_file_path + '\\Backup_word'):  # Перевіряємо чи існує папка Backup_word
        os.makedirs(
            source_file_path + '\\Backup_word')  # Якщо ні, то створюємо папку 'Backup_word' в директорії з виконуваним файлом
    now = datetime.datetime.now()  # Отримуємо поточну дату і час
    time = str(now.year) + '.' + str(now.month) + '.' + str(now.day) + '_' + str(now.hour) + '.' + str(
        now.minute)  # Підганяємо під потрібний нам формат
...
def search_coincidence(list):  # Функція пошуку співпадінь
    length = len(list)
    i = 0
    while i < length - 1:  # Переписав через while (замість for+range())
        x = i + 1  # через вихід за межі масиву, якщо в процесі удаляється хоча б один елемент
        while x < length:
